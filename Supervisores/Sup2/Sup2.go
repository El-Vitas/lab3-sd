...
const (
	brokerAddress2 = "localhost:50052" // Dirección del segundo Broker, si aplica
)

type Supervisor struct {
	id                   int
	lastKnownVectorClock map[string][]int32 // Updated to use []int32
}
...
func main() {
	...
	for {
		...
		case "AgregarProducto":
			...
			supervisor.sendCommand(brokerAddress2, command, []string{region, product, newValue})
		...
		case "BorrarProducto":
			...
			supervisor.sendCommand(brokerAddress2, command, []string{region, product})
		default:
